...
class BREADS(object):
    ...
    def __init__(self, config_file, seeds_file, negative_seeds, similarity, confidance):
        ...
        self.processed_tuples = list()      # to keep Tuple instances generated in the pre-processement
        self.seeds_by_iteration = dict()    # to control the semantic drift using the seeds from different iterations

        # Parallized version
        self.manager = multiprocessing.Manager()
        self.patterns = self.manager.list()          # to store the generated patterns
        self.candidate_tuples = self.manager.dict()  # to store the instances extracted with patterns
    ...
    def extract_instances(self, processed_tuples):
        ...
        while True:
            ...
            """ parte critica """
            # TODO: garantir que cada padrao soh estah a ser lido por um unico processo
    ...
    def init_bootstrapp(self, tuples):
        ...
        while self.curr_iteration <= self.config.number_iterations:
            ...
            if len(matched_tuples) == 0:
                ...
                #   Parallel version of extract_instances
                #
                #   defined at __init__():
                #   patterns  = shared list
                #   candidate_tuples = shared dict of lists
                ...
                for r in self.processed_tuples:
                    queue.put(r)
                processes = [multiprocessing.Process(target=self.extract_instances, args=queue) for _ in range(num_cpus)]

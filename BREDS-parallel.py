            ...
            if len(matched_tuples) == 0:
                ...
                if len(self.patterns) == 0:
                    ...
                    if len(child_patterns) > 1:
                        ...
                        # added list of merged patterns to main patterns structure
                        for p in new_list:
                            self.patterns.append(p)

                    elif len(child_patterns) == 1:
                        self.patterns.append(child_patterns[0])
    ...
    def cluster_tuples_parallel(self, patterns, matched_tuples, child_conn):
        ...
        for t in matched_tuples:
            ...
            for i in range(0, len(updated_patterns)):
                ...
                accept, score = self.similarity_all(t, extraction_pattern)
                """
                print multiprocessing.current_process().pid, "Tuple", t.bet_words
                for tlp in extraction_pattern.tuples:
                    print multiprocessing.current_process().pid, "Pattern", tlp.bet_words
                print multiprocessing.current_process().pid, score
                print
                """
                if accept is True and score > max_similarity:
                    max_similarity = score

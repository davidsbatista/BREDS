...
class BREDS(object):
    ...
    def init_bootstrapp(self, tuples):
        ...
        while self.curr_iteration <= self.config.number_iterations:
            ...
            if len(matched_tuples) == 0:
                ...
                # create copies of patterns
                results = [self.m.list() for _ in range(self.num_cpus)]
                ...
                processes = [multiprocessing.Process(target=self.find_instances(), args=(results[i], self.queue))
    ...
    def find_instances(self, results, patterns_queue):

        while True:
            try:
                extraction_pattern = patterns_queue.get_nowait()

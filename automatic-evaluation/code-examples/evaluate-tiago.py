...
def calculate_a(output, e1_type, e2_type, index):
    ...
    list_a = list(manager.list() for i in range(num_cpus))
    ...
    processes = [multiprocessing.Process(target=work_proc, args=(queue, l, e1_type, e2_type, index)) for l in list_a]
...
def work_proc(queue, list_a, e1_type, e2_type, index):
    while True:
        query_a(queue.get_nowait(), list_a, e1_type, e2_type, index)
